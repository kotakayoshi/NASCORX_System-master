...
class ymap(object):
    ...
    def makemap(self, LOrange=[90, 120], LOres=5, Vrange=[6, 8], Vres=0.2, Irange=[10, 30], Ires=10, logpath='/home/amigos/data/SIS/'):
        '''
        DESCRIPTION
        ================
        ...
        if os.path.isfile(logpath+'mixerYmap%s.csv'%(ts)) == 0:
            ...
            writecsv.writerow(['#UTC', 'LO[GHz]', 'Vmix[mV]', 'Imix[uA]', 'HOT[dB]', 'COLD[dBm], LOatt[%]'])
            f.close()
        else:
            pass
        cnt = 1
        ...
        for lo in Lo_list:
            ...
            for v in V_list:
                ...
                for i in I_list:
                    ...
                    self.load.rot_angle(speed=2000, angle=-90) # COLD
                    time.sleep(0.4)
                    Pcold = self.pm.measure(ch=1, resolution=3)
                    self.load.go_home(speed=2000) # HOT
                    print('********************')
                    ...
                    writecsv = csv.writer(f)
                    writecsv.writerow([dt, lf[0], Vmon, Imon, Phot, Pcold, i])
                    f.close()
                    cnt = cnt + 1
        self.mix.set_loatt(att=90, ch=0)
        self.sg.end_osci()
